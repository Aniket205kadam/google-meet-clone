...
import { loadUserInfo } from "../../store/userInfoSlice";
...
const Home = () => {
  ...
  const userFromRedux = useSelector((state) => state.userInfo);
  ...
  useClickOutside(sidePanelRef, () => setIsSidePanelOpen(false));
  useClickOutside(accountInfoRef, () => setIsAccountInfoOpen(false));
  ...
  const fetchUser = async () => {
    try {
      ...
      toast.error(
        "Failed to load your account info. Refresh the page or log in again."
      );
    }
  };
  ...
  const getCallHistory = async () => {
    try {
      const response = await callService.fetchCallHistory(currentPageNo, 6);
      setCallHistory((ch) => [...ch, ...response.content]);
      ...
      toast.error("Failed to fetch call history. Try again..!");
    }
  };
  ...
  const handlePageReload = () => {
    if (!userFromRedux?.id) {
      ...
      setConnectedUser({ ...userFromRedux });
    }
  };
  ...
  return (
    <div className="home-page">
      <div className="search-container">
        <div className="more-options" onClick={() => setIsSidePanelOpen(true)}>
          <svg
        ...
        <div className="search-btn" onClick={() => navigate("/search-users")}>
          Search contacts
        </div>
        <div className="enter-code" onClick={() => navigate("/join-with-code")}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
            viewBox="0 -960 960 960"
      ...
      <div className="add-call" onClick={() => navigate("/create-call")}>
        <div className="add-call-icon">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
            viewBox="0 -960 960 960"
      ...
      {loading ? (
        <div className="latest-activity-loading">
          <PageDataLoader />
        </div>
      ) : (
        <div className="latest-activity">
          ...
          {callHistory.length === 0 && (
            <div className="no-latest-activity">
              <div className="no-activity-icon">
                ...
                >
                  ...
                  />
                  <polygon
                    points="286.64 284.88 312.79 284.88 312.79 326.36 283.96 316.89 286.64 284.88"
                    fill="#f3a3a6"
                    strokeWidth="0"
                  />
                  <path
                  ...
                  />
                  <path
                    id="uuid-3fbf081d-9ff2-4e55-8986-b74bb868839d-47-155-46-48-44-44-85-89-39-99-287"
                  ...
                  />
                  <polygon
                    points="538.71 164.58 515.11 164.58 515.11 202.02 541.13 193.47 538.71 164.58"
                    fill="#f3a3a6"
                    strokeWidth="0"
                  />
                  <path
                  ...
                  <rect
                    ...
                    width="20.94"
                    height="29.71"
                    transform="translate(-196.29 498.38) rotate(-39.6)"
                    fill="#f3a3a6"
                    strokeWidth="0"
                  />
                  <path
                  ...
                  />
                  <path
                    d="m492.71,319.1l31.92,237.43h23.94l15.96-237.43h-71.83,0Z"
                    fill="#2f2e43"
                    strokeWidth="0"
                  />
                  <polygon
                    points="554.55 448.79 602.22 510.64 581.82 527.49 500.01 449.68 492.7 319.1 507.33 332.18 554.55 448.79"
                    fill="#2f2e43"
                    strokeWidth="0"
                  />
                  <path
                    d="m507.33,179.43l-14.63,139.67h71.83l5.57-45.33c3.43-28-2.28-56.36-16.28-80.84l-11.02-19.26-35.47,5.76h0Z"
                    fill="#d5d5d6"
                    strokeWidth="0"
                  />
                  <path
                    id="uuid-6d861307-5b4f-42f9-9fe9-ab52d0b2e64d-48-156-47-49-45-45-86-90-40-100-288"
          ...
          {callHistory.length > 0 && (
            <div className="call-history-container">
              ...
              {!isLastPage && (
                <button
                  className="show-more-call-history"
                  onClick={() => {
                    setMoreLoading(true);
      ...
      <SidePanel isSidePanelOpen={isSidePanelOpen} ref={sidePanelRef} />

      <div
        className="blur-background"
        style={isSidePanelOpen || isAccountInfoOpen ? {} : { display: "none" }}
      ></div>
      ...
      {isAccountInfoOpen && (
        ...
        >
          <AccountInfo
            profileImg={connectedUser.profile}
